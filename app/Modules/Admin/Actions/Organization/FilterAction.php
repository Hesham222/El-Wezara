...
class FilterAction
{
    ...
    {
        return Organization::with(['services'])->when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->select(['id','name', 'address','deleted_by','deleted_at', 'created_at'])
        ...
        ->when($request->input('column') && $request->input('value'), function ($query) use ($request){
            return $query->when($request->input('column') == 'deletedBy' , function ($query) use ($request){
                ...
                })->when($request->input('column') == 'address', function ($query) use ($request){
                    return $query->where('address', 'like', '%' . $request->input('value') . '%');
                });
        });
