...
@if(request()->input('view')=='trash')
  <x-slot name="pageTitle">المنظمات | سله المهملات</x-slot name="pageTitle">
  @section('organizations-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">المنظمات | عرض</x-slot name="pageTitle">
  @section('organizations-view-active', 'm-menu__item--active')
@endif
        ...
        <div class="m-portlet m-portlet--mobile">
              ...
              <ul class="m-portlet__nav">
                  @if(checkAdminPermission(auth('admin')->user()->role->permissions->pluck('permission_id')->toArray(),'Organization-Module'))
                      <li class="m-portlet__nav-item">
                          <a href="{{route('admins.organization.create')}}"
                             class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                    <span>
                      <i class="la la-plus">
                      </i>
                      <span>اضافة منظمه جديده</span>
                    </span>
                          </a>
                      </li>
                  @endif
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Admin::organizations.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('admins.organization.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('admins.organization.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
  <!-- external JS -->
