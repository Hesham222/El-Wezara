...
<tr id="tableRecord-{{$record->id}}">
    ...
    @if(request()->query('view')=='trash')
        ...
        onclick="injectModalData('{{$record->id}}', '{{route('admins.role.restore')}}', 'confirm-password-form', 'POST')"
        >
        ...
        <a 
            ...
            data-toggle="modal" 
            data-target="#confirm-password-modal"
            onclick="injectModalData('{{$record->id}}', '{{route('admins.role.destroy', $record->id)}}', 'confirm-password-form', 'DELETE')"
        >
            <i class="fa fa-trash" style="color: #fff"></i>
        </a> 
    @else
        <a 
            href="{{route('admins.role.edit',$record->id)}}"
            title="تعديل"
            class="btn btn-sm btn-primary">
        ...
        @if($record->id !==1 && $record->id != auth('admin')->user()->id)
            ...
            data-toggle="modal" 
            data-target="#confirm-password-modal"
            onclick="injectModalData('{{$record->id}}', '{{route('admins.role.trash')}}', 'confirm-password-form', 'POST')" >
            <i class="fa fa-trash" style="color: #fff"></i>
            </a> 
        @endif
    @endif
