          ...
          <div class="m-portlet__body">
            <div class="table-responsive">
                <section class="content">
                  <form method="POST" action="{{route('admins.role.update', $record->id)}}"
                              ...
                              <div class="col-lg-6">
                                  ...
                                  <select name="permissions[]" multiple required="" class="form-control m-input m-input--square" id="exampleSelect1" >
                                      @foreach($permissions as $permission)
                                          <option @if($permission->id == $record->permissions->where('permission_id',$permission->id)->pluck('permission_id')->first()) selected @endif value="{{ $permission->id }}">{{ $permission->name }}</option>
                                      @endforeach
                                  </select>
                                  @error('permissions')
                                  <span class="invalid-feedback" role="alert">
