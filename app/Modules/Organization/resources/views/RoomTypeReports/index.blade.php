<x-organization::layout>
  <x-slot name="pageTitle">تقرير احصائيات انواع الغرف | عرض</x-slot name="pageTitle">
    @section('hotel-reports-active', 'm-menu__item--active m-menu__item--open')
    @section('RoomTypeReports-view-active', 'm-menu__item--active')
        ...
        <div class="m-portlet m-portlet--mobile">
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::RoomTypeReports.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.RoomTypeReport.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.RoomTypeReport.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val()+ '&date_from=' + $('#date_from').val()+'&date_to=' + $('#date_to').val());
        });
    });
  </script>
</x-slot>
