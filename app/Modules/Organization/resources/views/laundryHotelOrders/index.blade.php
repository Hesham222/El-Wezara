<x-organization::layout>
    @if(request()->input('view')=='trash')
        <x-slot name="pageTitle">طلبات اسكان  | سله المهملات</x-slot name="pageTitle">
        @section('laundryHotelOrders-trash-active', 'm-menu__item--active')
    @else
        <x-slot name="pageTitle">طلبات اسكان | عرض</x-slot name="pageTitle">
        @section('laundryHotelOrders-view-active', 'm-menu__item--active')
    @endif
        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                <li class="m-portlet__nav-item">
                  <a href="{{route('organizations.laundryHotelOrder.create')}}"
                    class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                    <span>
                      <i class="la la-plus">
                      </i>
                      <span>أضف طلب اسكان جديد</span>
                    </span>
                  </a>
                </li>
              </ul>
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::laundryHotelOrders.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.laundryHotelOrder.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.laundryHotelOrder.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
