<x-organization::layout>
    @if(request()->input('view')=='trash')
        <x-slot name="pageTitle">@lang('Organization::organization.employeeDeduction') | @lang('Organization::organization.trash')</x-slot name="pageTitle">
        @section('employeeDeduction-trash-active', 'm-menu__item--active')
    @else
        <x-slot name="pageTitle">@lang('Organization::organization.employeeDeduction') | @lang('Organization::organization.view')</x-slot name="pageTitle">
        @section('employeeDeduction-indexDepartment-active', 'm-menu__item--active')
    @endif
    @section('employeeDeduction-active', 'm-menu__item--active m-menu__item--open')
        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__body">
                <section class="content">
                    @include('Organization::employeeDeductions.components.filterForm')
...
{{--                                    <th>@lang('Organization::organization.actions')</th>--}}
    ...
    <x-slot name="scripts">
        <script>
            $(function () {
                render("{!! route('organizations.employeeDeduction.empDeductionData',['id'=>$emp->id,'view' => request()->input('view',0)]) !!}");
                /**
                ...
                $('#searchButton').on('click', function (event) {
                    event.stopPropagation();
                    render("{!! route('organizations.employeeDeduction.empDeductionData',['id'=>$emp->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
                });
            });
        </script>
        <!-- external JS -->
