...
@section('rent-contracts-active', 'm-menu__item--active m-menu__item--open')
    @include('Organization::rentContractPayments.components.confirm_rent_payment')
  <x-slot name="style">
    <!-- Some styles -->
  </x-slot>
    <!-- Start page content -->
        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__head-tools">
               <ul class="m-portlet__nav">
                  <li class="m-portlet__nav-item">
                      <a href="{{ route('organizations.rentContractPayment.index',$id).'?view=view' }}" class="btn btn-warning m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                      <span>
                         <i class="la la-refresh"></i>
                            <span>اعادة تحميل</span>
                        </span>
                        </a>
                    </li>
                  </ul>
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::rentContractPayments.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.rentContractPayment.data',[$id,'view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.rentContractPayment.data',[$id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
