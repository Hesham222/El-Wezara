<x-organization::layout>
  ...
  <x-slot name="pageTitle">مخازن الفنادق  | سله المهملات</x-slot name="pageTitle">
  @section('hotelInventories-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">مخازن الفنادق | عرض</x-slot name="pageTitle">
  @section('hotelInventories-view-active', 'm-menu__item--active')
@endif
...
{{--                  <a href="{{route('organizations.hotelInventory.create')}}"--}}
...
{{--                  </a>--}}
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::hotelInventories.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.hotelInventory.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.hotelInventory.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
