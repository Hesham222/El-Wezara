...
<tr id="tableRecord-{{$record->id}}">
    ...
    <td>{{$record->hotel->name? $record->hotel->name:"لا يوجد"}}</td>
    ...
    @else
        @if ($record->quantity > 0)
                @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'HotelInventory-Consumption'))

                <a class="btn btn-sm btn-primary"
                   data-id ="{{$record->id}}"
                   title="جرد"
                   href="{{route('organizations.hotelInventory.consumption',$record->id)}}"
                >
