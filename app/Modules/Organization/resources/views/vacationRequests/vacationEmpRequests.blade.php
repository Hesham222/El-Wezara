...
{{--                                    <th>@lang('Organization::organization.actions')</th>--}}
    ...
    <x-slot name="scripts">
        <script>
            $(function () {
                render("{!! route('organizations.vacationRequest.empVacationData',['id'=>$emp->id,'view' => request()->input('view',0)]) !!}");
                /**
                ...
                $('#searchButton').on('click', function (event) {
                    event.stopPropagation();
                    render("{!! route('organizations.vacationRequest.empVacationData',['id'=>$emp->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
                });
            });
        </script>
        <!-- external JS -->
