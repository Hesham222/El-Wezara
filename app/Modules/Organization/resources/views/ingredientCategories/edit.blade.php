<x-organization::layout>
 <x-slot name="pageTitle">فئات مكونات الوجبات | تعديل</x-slot name="pageTitle">
          ...
          <div class="m-portlet__body">
            <div class="table-responsive">
                <section class="content">
                  <form method="POST" action="{{route('organizations.ingredientCategory.update', $record->id)}}"
                      ...
                      <div class="m-portlet__body">
                          <div class="form-group m-form__group row">
                              <div class="col-lg-6">
                                  ...
                                  <div class="col-lg-6">
                                      <label>أختر فئه مكون الوجبات:</label>
                                      <select name="parent_id" required="" class="form-control m-input m-input--square" id="exampleSelect1">
                                          <option value="0">لا يوجد</option>
                                          @foreach($categories as $category)
                                              <option @if($record->parent_id) selected @endif value="{{ $category->id }}">{{ $category->name }}</option>
                                          @endforeach
                                      </select>
                                      @error('parent_id')
                                      <span class="invalid-feedback" role="alert">
