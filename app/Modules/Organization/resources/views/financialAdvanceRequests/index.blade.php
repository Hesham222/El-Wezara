<x-organization::layout>
    @if(request()->input('view')=='trash')
        <x-slot name="pageTitle">@lang('Organization::organization.financialAdvanceRequest') | @lang('Organization::organization.trash')</x-slot name="pageTitle">
        @section('financialAdvanceRequest-trash-active', 'm-menu__item--active')
    @else
        <x-slot name="pageTitle">@lang('Organization::organization.financialAdvanceRequest') | @lang('Organization::organization.view')</x-slot name="pageTitle">
        @section('financialAdvanceRequest-view-active', 'm-menu__item--active')
    @endif
        ...
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-tools">
                    <ul class="m-portlet__nav">
                        @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'FinancialRequest-Add'))

                        <li class="m-portlet__nav-item">
                            <a href="{{route('organizations.financialAdvanceRequest.create')}}"
                               class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                    <span>
                      <i class="la la-plus">
                      </i>
                      <span>@lang('Organization::organization.financialAdvanceRequest') @lang('Organization::organization.new')</span>
                    </span>
                            </a>
                        </li>
                        @endif
            ...
            <div class="m-portlet__body">
                <section class="content">
                    @include('Organization::financialAdvanceRequests.components.filterForm')
    ...
    <x-slot name="scripts">
        <script>
            $(function () {
                render("{!! route('organizations.financialAdvanceRequest.data',['view' => request()->input('view',0)]) !!}");
                /**
                ...
                $('#searchButton').on('click', function (event) {
                    event.stopPropagation();
                    render("{!! route('organizations.financialAdvanceRequest.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
                });
            });
        </script>
        <!-- external JS -->
