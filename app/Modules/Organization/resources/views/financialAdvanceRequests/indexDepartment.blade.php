<x-organization::layout>
    ...
    @else
        ...
        @section('financialAdvanceRequest-indexDepartment-active', 'm-menu__item--active')
    @endif
    ...
    <x-slot name="scripts">
        <script>
            $(function () {
                render("{!! route('organizations.financialAdvanceRequest.dataDepartment',['id'=>auth('organization_admin')->user()->employee->department->id,'view' => request()->input('view',0)]) !!}");
                /**
                ...
                $('#searchButton').on('click', function (event) {
                    event.stopPropagation();
                    render("{!! route('organizations.financialAdvanceRequest.dataDepartment',['id'=>auth('organization_admin')->user()->employee->department->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
                });
            });
        </script>
        <!-- external JS -->
