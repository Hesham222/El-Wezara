...
@else
  <x-slot name="pageTitle">مناطق التحضير | جرد</x-slot name="pageTitle">
      ...
      <div class="m-content">
        <div class="m-portlet m-portlet--mobile">
          <div class="m-portlet__head">
           ...
           @if(count($area->PreparationAreaInventories) > 0)
            <div class="m-portlet__head-tools">
             جرودات منطقة التحضير
                [{{$area->name}}]
                <br>
                <a href="{{route('organizations.PreparationAreaStocking.create',$area->id)}}" class="btn btn-primary">عمل جرد جديد</a>
            </div>
               @endif
          </div>
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::PreparationAreaStockings.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.PreparationAreaStocking.data',['id'=>$area->id,'view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.PreparationAreaStocking.data',['id'=>$area->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
