<x-organization::layout>
    @if(request()->input('view')=='trash')
        <x-slot name="pageTitle">@lang('Organization::organization.employeeJob') | @lang('Organization::organization.trash')</x-slot name="pageTitle">
        @section('employeeJob-trash-active', 'm-menu__item--active')
    @else
        <x-slot name="pageTitle">@lang('Organization::organization.employeeJob') | @lang('Organization::organization.view')</x-slot name="pageTitle">
        @section('employeeJob-view-active', 'm-menu__item--active')
    @endif
        ...
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-tools">
                    <ul class="m-portlet__nav">
                        @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'EmployeeJob-Add'))
                            <li class="m-portlet__nav-item">
                            <a href="{{route('organizations.employeeJob.create')}}"
                               class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                    <span>
                      <i class="la la-plus">
                      </i>
                      <span>@lang('Organization::organization.employeeJob') @lang('Organization::organization.new')</span>
                    </span>
                            </a>
                        </li>
                        @endif
            ...
            <div class="m-portlet__body">
                <section class="content">
                    @include('Organization::employeeJobs.components.filterForm')
...
{{--                                    <th>@lang('Organization::organization.arabicDescription')</th>--}}
                                    <th>@lang('Organization::organization.employee_vacation_balance')</th>
    ...
    <x-slot name="scripts">
        <script>
            $(function () {
                render("{!! route('organizations.employeeJob.data',['view' => request()->input('view',0)]) !!}");
                /**
                ...
                $('#searchButton').on('click', function (event) {
                    event.stopPropagation();
                    render("{!! route('organizations.employeeJob.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
                });
            });
        </script>
        <!-- external JS -->
