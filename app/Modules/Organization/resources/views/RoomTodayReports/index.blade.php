<x-organization::layout>
  <x-slot name="pageTitle">تقرير إحصائيات جميع الغرف المحجوزه اليوم | عرض</x-slot name="pageTitle">
    @section('hotel-reports-active', 'm-menu__item--active m-menu__item--open')
    @section('RoomBalances-view-active', 'm-menu__item--active')
        ...
        <div class="d-flex align-items-center">
          <div class="mr-auto">
            <h3 class="m-subheader__title ">
                تقرير إحصائيات جميع الغرف المحجوزه اليوم
            </h3>
          </div>
        </div>
      </div>
        ...
        <div class="m-portlet m-portlet--mobile">
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::RoomTodayReports.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.RoomTodayReport.data',['view' => request()->input('view',0),'date' => $date]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.RoomTodayReport.data',['view' => request()->input('view',0),'date' => $date]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val()+ '&date_from=' + $('#date_from').val()+'&date_to=' + $('#date_to').val());
        });
    });
  </script>
</x-slot>
