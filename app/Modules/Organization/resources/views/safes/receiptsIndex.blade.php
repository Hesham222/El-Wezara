<x-organization::layout>
  <x-slot name="pageTitle">  ايصالات الخزنة | عرض</x-slot name="pageTitle">
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.safe.receiptsData',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.safe.receiptsData',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
