<x-organization::layout>
  <x-slot name="pageTitle">تقرير عمليات الدفع | عرض</x-slot name="pageTitle">
    @section('sport-reports-active', 'm-menu__item--active m-menu__item--open')
    @section('paymentReports-view-active', 'm-menu__item--active')
        ...
        <div class="m-portlet m-portlet--mobile">
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::paymentReports.components.filterForm')
                    ...
                    <section class="content table-responsive">
                      <table id="data-table" class="table table-striped- table-bordered table-hover table-checkable" >
                          ...
                          <tr>
                              ...
                              <th>الرقم التعريفي للدفع </th>
                              <th>الرقم التعريفي للأشتراك </th>
                              <th>اسم التدريب</th>
                              <th>مبلغ الدفع</th>
                              <th>تاريخ الدفع</th>
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.paymentReport.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.paymentReport.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val()+'&day=' + $('#dayCol').val()+'&date_from=' + $('#date_from').val()+'&date_to=' + $('#date_to').val()+'&subscriber=' + $('#subscriberColumn').val()+'&phone=' + $('#phoneColumn').val()+'&payment=' + $('#paymentColumn').val()+'&subscription=' + $('#subscriptionColumn').val()+'&training=' + $('#trainingColumn').val());
        });
    });
  </script>
</x-slot>
