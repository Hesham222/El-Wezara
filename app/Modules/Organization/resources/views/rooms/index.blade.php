<x-organization::layout>
  ...
  <x-slot name="pageTitle">الغرف  | سله المهملات</x-slot name="pageTitle">
  @section('rooms-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">الغرف | عرض</x-slot name="pageTitle">
  @section('rooms-view-active', 'm-menu__item--active')
@endif
...
{{--                  <a href="{{route('organizations.room.create')}}"--}}
{{--                    class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">--}}
{{--                    <span>--}}
{{--                      <i class="la la-plus">--}}
{{--                      </i>--}}
{{--                      <span>أضف فندق جديد</span>--}}
{{--                    </span>--}}
{{--                  </a>--}}
{{--                </li>--}}
              </ul>
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::rooms.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.room.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.room.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
