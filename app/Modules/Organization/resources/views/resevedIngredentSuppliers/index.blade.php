<x-organization::layout>
  <x-slot name="pageTitle">تقارير استلامات الاصناف بالموردين  | عرض</x-slot name="pageTitle">
  @section('pointOfSales-sheets-active', 'm-menu__item--active')
  <x-slot name="style">
    <!-- Some styles -->
  </x-slot>
    <!-- Start page content -->
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.resevedIngredentSupplier.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.resevedIngredentSupplier.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
