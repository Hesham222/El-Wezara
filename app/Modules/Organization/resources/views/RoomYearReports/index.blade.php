<x-organization::layout>
  <x-slot name="pageTitle"> تقرير السنويه للحجوزات | عرض</x-slot name="pageTitle">
        ...
        <div class="m-portlet m-portlet--mobile">
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::RoomYearReports.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.RoomYearReport.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.RoomYearReport.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val()+ '&date_from=' + $('#date_from').val()+'&date_to=' + $('#date_to').val());
        });
    });
  </script>
</x-slot>
