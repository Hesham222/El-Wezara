<x-organization::layout>
  ...
  <x-slot name="pageTitle">الاشتراكات  | سله المهملات</x-slot name="pageTitle">
  @section('subscriptions-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">الاشتراكات | عرض</x-slot name="pageTitle">
        ...
        <div class="m-portlet m-portlet--mobile">
              ...
              <ul class="m-portlet__nav">
                  @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'Subscription-Add'))
                        <li class="m-portlet__nav-item">
                          <a href="{{route('organizations.subscription.create')}}"
                            class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                            <span>
                              <i class="la la-plus">
                              </i>
                              <span>أضف اشتراك جديد</span>
                            </span>
                          </a>
                        </li>
                  @endif
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::subscriptions.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.subscription.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.subscription.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val()+ '&subscriber=' + $('#subscriberColumn').val()+ '&training=' + $('#trainingColumn').val()+ '&price_name=' + $('#price_nameColumn').val());
        });
    });
  </script>
</x-slot>
