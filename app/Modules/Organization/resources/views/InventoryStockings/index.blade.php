...
@else
  <x-slot name="pageTitle">المخزن الرئيسي | جرد</x-slot name="pageTitle">
        ...
        <div class="m-portlet m-portlet--mobile">
          <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
             جرودات المخزن الرئيسي
                [{{$area->name}}]
                <br>
                <a href="{{route('organizations.InventoryStocking.create',$area->id)}}" class="btn btn-primary">عمل جرد جديد</a>
            </div>
          </div>
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::InventoryStockings.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.InventoryStocking.data',['id'=>$area->id,'view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.InventoryStocking.data',['id'=>$area->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
