<x-organization::layout>
  ...
  @section('laundryInventories-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">المخازن | عرض</x-slot name="pageTitle">
  @section('laundryInventories-view-active', 'm-menu__item--active')
@endif
@section('laundryInventories-active', 'm-menu__item--active m-menu__item--open')
        ...
        <div class="m-portlet m-portlet--mobile">
          <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                  @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'LaundryInventory-Add'))

                  <li class="m-portlet__nav-item">
                  <a href="{{route('organizations.laundryInventory.create')}}"
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::laundryInventories.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.laundryInventory.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.laundryInventory.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
