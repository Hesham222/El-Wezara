...
<tr id="tableRecord-{{$record->id}}">
    ...
    <td>{{$record->laundry->name? $record->laundry->name:"لا يوجد"}}</td>
    ...
    @else
        @if ($record->quantity > 0)
                @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'LaundryInventory-consumption'))

                <a class="btn btn-sm btn-primary"
                   data-id ="{{$record->id}}"
                   title="جرد"
                   href="{{route('organizations.laundryInventory.consumption',$record->id)}}"
                >
