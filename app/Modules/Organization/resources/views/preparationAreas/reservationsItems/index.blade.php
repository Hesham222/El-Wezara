<x-organization::layout>
  ...
  <x-slot name="pageTitle"> المنتججات المطلوب تجهيزها للحجوزات  | سله المهملات</x-slot name="pageTitle">
  @section('preparationAreas-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">  المنتججات المطلوب تجهيزها للحجوزات | عرض</x-slot name="pageTitle">
        ...
        <div class="d-flex align-items-center">
          <div class="mr-auto">
            <h3 class="m-subheader__title ">
                     المنتججات المطلوب تجهيزها للحجوزات
            </h3>
          </div>
        </div>
      </div>
        ...
        <div class="m-portlet m-portlet--mobile">
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::preparationAreas.reservationsItems.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.preparationArea.reservations.items.data',['id'=>$record->id,'view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.preparationArea.reservations.items.data',['id'=>$record->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
