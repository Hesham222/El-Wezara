<x-organization::layout>
  ...
  <x-slot name="pageTitle">حجوزات الفنادق  | سله المهملات</x-slot name="pageTitle">
  @section('hotelReservations-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">حجوزات الفنادق | عرض</x-slot name="pageTitle">
          ...
          <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                  @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'HotelReservation-Add'))

                  <li class="m-portlet__nav-item">
                  <a href="{{route('organizations.hotelReservation.create')}}"
                    class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                    <span>
                      <i class="la la-plus">
                      </i>
                      <span>أضف حجز جديد</span>
                    </span>
                  </a>
                </li>
                  @endif
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.hotelReservation.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.hotelReservation.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
