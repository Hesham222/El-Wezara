<x-organization::layout>
  <x-slot name="pageTitle">استهلاك منطقه التحضير تفصيليا | عرض</x-slot name="pageTitle">
  @section('AreaConsumptions-active', 'm-menu__item--active')
  @section('AreaConsumptions-view-active', 'm-menu__item--active')
    @section('AreaConsumptions-active', 'm-menu__item--active m-menu__item--open')
        ...
        <div class="m-portlet m-portlet--mobile">
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::AreaConsumptionDetail.components.AreaConsumptionDetailFilterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.AreaConsumptionDetail.data',['id'=>$record->id,'view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.AreaConsumptionDetail.data',['id'=>$record->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
