        ...
        @if($data)
            ...
            @else
                @if(filter_var($data->attachment, FILTER_VALIDATE_URL))
                    <img src="{{ $data->attachment }}" alt="image-not-uploaded" width="100">
                @else
                    <img src="{{asset('storage'.DS().$data->attachment)}}" alt="image-not-uploaded" width="100">
                @endif
            @endif
            <td>
                <input
...
<table>
    @if(isset($customer_information))
        @foreach ($customer_information as $information)
    <thead>
