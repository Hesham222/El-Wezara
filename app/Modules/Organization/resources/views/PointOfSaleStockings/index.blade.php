          ...
          <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
             جرودات نقط البيع
                [{{$area->name}}]
                <br>
                <a href="{{route('organizations.POStocking.create',$area->id)}}" class="btn btn-primary">عمل جرد جديد</a>
            </div>
          </div>
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::PointOfSaleStockings.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.POStocking.data',['id'=>$area->id,'view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.POStocking.data',['id'=>$area->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
