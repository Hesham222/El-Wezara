<x-organization::layout>
 <x-slot name="pageTitle">القيد المحاسبي | اضف</x-slot name="pageTitle">
 @section('journalEntries-active', 'm-menu__item--active m-menu__item--open')
 @section('journalEntries-create-active', 'm-menu__item--active')
          ...
          <div class="m-portlet__body">
            <div class="table-responsive">
                <section class="content">
                  <form method="POST" action="{{route('organizations.journalEntry.store')}}" enctype="multipart/form-data"
                        class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed" id="invoces">
                      @csrf
                      @method('post')
                      <div class="m-portlet__body">
                          ...
                          @if(isset($invoicesIds))


                          <input type="hidden" name="invoicesIds[]" value="{{$invoicesIds}}">

                            @if($invoiceType == 'sportsActivities')
                          <input type="hidden" name="modelType" value="Payment">
                          @elseif($invoiceType == 'RentContractPayment')
                          <input type="hidden" name="modelType" value="RentContractPayment">
                          @elseif($invoiceType == 'HotelReservationPayment')
                          <input type="hidden" name="modelType" value="HotelReservationPayment">
                          @elseif($invoiceType == 'GateShiftSheet')
                          <input type="hidden" name="modelType" value="GateShiftSheet">
                           @elseif($invoiceType == 'ReservationPayment')
                          <input type="hidden" name="modelType" value="ReservationPayment">
                          @elseif($invoiceType == 'LaundryOrderPayment')
                          <input type="hidden" name="modelType" value="LaundryOrderPayment">
                          @elseif($invoiceType == 'OrderPayment')
                          <input type="hidden" name="modelType" value="OrderPayment">
                          @endif
                          @endif
                          <div id="debit">
                              @include('Organization::journalEntries.components.debit.table')
                          </div>
                          <div id="credit">
                              @include('Organization::journalEntries.components.credit.table')
                          </div>
