<x-organization::layout>
  <x-slot name="pageTitle">أرصدة المشتركين المتبقية | عرض</x-slot name="pageTitle">
    @section('sport-reports-active', 'm-menu__item--active m-menu__item--open')
    @section('subscriberBalances-view-active', 'm-menu__item--active')
        ...
        <div class="m-portlet m-portlet--mobile">
          ...
          <div class="m-portlet__body">
             <section class="content">
                 @include('Organization::subscriberBalances.components.filterForm')
                          ...
                          @if(count($records))
                              ...
                              @foreach($records as $record)
                                      ...
                                      <tr id="tableRecord-{{$record->id}}">
                                          ...
                                          <td>{{$record->Subscriptions?$record->Subscriptions->sum('payment_balance'):"لايوجد"}}</td>

                                          <td>
                                              <a
                                                  href="{{route('organizations.subscriberBalance.show',$record->id)}}"
                                                  title="اظهر الاشتراكات"
                                                  style="margin:5px"
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.subscriberBalance.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.subscriberBalance.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val()+'&day=' + $('#dayCol').val());
        });
    });
  </script>
</x-slot>
