<x-organization::layout>
 <x-slot name="pageTitle">ادارة البوابات | اضف</x-slot name="pageTitle">
 @section('gate-shifts-active', 'm-menu__item--active m-menu__item--open')
 @section('gate-shifts-create-active', 'm-menu__item--active')
  <x-slot name="style">
  <!-- Some styles -->
    <style>
        .invalid-feedback {
          ...
          <div class="m-portlet__body">
            <div class="table-responsive">
                <section class="content">
                  <form method="POST" action="{{route('organizations.gateShift.store')}}"
                          ...
                          <div class="form-group m-form__group row">
                              <div class="col-lg-6">
                                  ...
                                  <select name="day" required="" class="form-control m-input m-input--square" id="exampleSelect1">
                                      @foreach($days as $day)
                                          <option @if(old('day') == $day->id) selected @endif value="{{ $day->id }}">{{ $day->name }}</option>
                                      @endforeach
                                  </select>
                                  @error('day')
                              ...
                              <div class="col-lg-6">
                                  ...
                                  <select name="admins[]" required="" class="form-control m-input m-input--square" id="exampleSelect1" multiple>
                                      @foreach($admins as $admin)
                                          <option @if(old('admins') == $admin->id) selected @endif value="{{ $admin->id }}">{{ $admin->name }} - {{ $admin->phone }}</option>
                                      @endforeach
                                  </select>
                                  @error('admins')
                                  <span class="invalid-feedback" role="alert">
