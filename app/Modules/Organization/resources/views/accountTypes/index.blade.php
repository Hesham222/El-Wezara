<x-organization::layout>
  ...
  <x-slot name="pageTitle">انواع الحسابات  | سله المهملات</x-slot name="pageTitle">
  @section('accountTypes-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">انواع الحسابات | عرض</x-slot name="pageTitle">
  @section('accountTypes-view-active', 'm-menu__item--active')
@endif
@section('accountTypes-active', 'm-menu__item--active m-menu__item--open')
        ...
        <div class="m-portlet m-portlet--mobile">
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::accountTypes.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.accountType.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.accountType.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
