<x-organization::layout>
    @if(request()->input('view')=='trash')
        <x-slot name="pageTitle">التقرير الثلاثي  | سله المهملات</x-slot name="pageTitle">
        @section('triple-sheet-report-active', 'm-menu__item--active')
    @else
        <x-slot name="pageTitle">التقرير الثلاثي | عرض</x-slot name="pageTitle">
        @section('triple-sheet-report-active', 'm-menu__item--active')
    @endif
        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__body">
                <section class="content">
                    @include('Organization::reports.components.triple.filterForm')
    ...
    <x-slot name="scripts">
        <script>
            $(function () {
                render("{!! route('organizations.report.data_triple') !!}");
                /**
                ...
                $('#searchButton').on('click', function (event) {
                    event.stopPropagation();
                    render("{!! route('organizations.report.data_triple',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
                });
            });
        </script>
    </x-slot>
