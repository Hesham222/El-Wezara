<x-organization::layout>
    @if(request()->input('view')=='trash')
        <x-slot name="pageTitle">تقارير عمليات الدفع  | سله المهملات</x-slot name="pageTitle">
        @section('payments-transactions-Report-active', 'm-menu__item--active')
    @else
        <x-slot name="pageTitle">تقارير عمليات الدفع | عرض</x-slot name="pageTitle">
        @section('payments-transactions-Report-active', 'm-menu__item--active')
    @endif
    @section('payments-transactions-Report-active', 'm-menu__item--active m-menu__item--open')
        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__body">
                <section class="content">
                    @include('Organization::reports.components.transactions.filterForm')
    ...
    <x-slot name="scripts">
        <script>
            $(function () {
                render("{!! route('organizations.report.data_transactions') !!}");
                /**
                ...
                $('#searchButton').on('click', function (event) {
                    event.stopPropagation();
                    render("{!! route('organizations.report.data_transactions',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
                });
            });
        </script>
    </x-slot>
