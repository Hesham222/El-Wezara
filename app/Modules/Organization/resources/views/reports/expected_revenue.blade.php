<x-organization::layout>
    @if(request()->input('view')=='trash')
        <x-slot name="pageTitle">تقارير الارباح المتوقعه  | سله المهملات</x-slot name="pageTitle">
        @section('customers-remaining-amounts-active', 'm-menu__item--active')
    @else
        <x-slot name="pageTitle">تقارير الارباح المتوقعه | عرض</x-slot name="pageTitle">
        @section('customers-remaining-amounts-active', 'm-menu__item--active')
    @endif
        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__body">
                <section class="content">
                    @include('Organization::reports.components.expected_revenues.filterForm')
    ...
    <x-slot name="scripts">
        <script>
            $(function () {
                render("{!! route('organizations.report.data_expected_revenue') !!}");
                /**
                ...
                $('#searchButton').on('click', function (event) {
                    event.stopPropagation();
                    render("{!! route('organizations.report.data_expected_revenue',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
                });
            });
        </script>
    </x-slot>
