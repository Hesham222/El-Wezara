<x-organization::layout>
 <x-slot name="pageTitle"> نقاط البيع  | تعديل</x-slot name="pageTitle">
            ...
            <div class="m-portlet__body">
                <div class="table-responsive">
                    <section class="content">
                        <form method="POST" action="{{route('organizations.pointOfSale.update', $record->id)}}"enctype="multipart/form-data"
                ...
                <div class="form-group m-form__group row">
                    <div class="col-lg-6">
                        ...
                        <select name="items[]" required="" class="form-control m-input m-input--square" id="exampleSelect1" multiple>
                            @foreach($items as $item)
                                <option @if(!is_null($record->PointOfSaleItems->where('item_id',$item->id)->first())) selected @endif value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('items')
                        <span class="invalid-feedback" role="alert">
                    ...
                    <div class="col-lg-6">
                        ...
                        <select name="employees[]" required="" class="form-control m-input m-input--square" id="exampleSelect1" multiple>
                            @foreach($employees as $employee)
                                <option @if(!is_null($record->PointOfSaleEmployees->where('employee_id',$employee->id)->first())) selected @endif value="{{ $employee->id }}">{{ $employee->name }}</option>
