<x-organization::layout>
  <x-slot name="pageTitle">تقاريرمكونات التجهيزات  | عرض</x-slot name="pageTitle">
  @section('ItemComponents-active', 'm-menu__item--active')
  @section('ItemComponents-view-active', 'm-menu__item--active')
  <x-slot name="style">
    <!-- Some styles -->
  </x-slot>
    <!-- Start page content -->
        ...
        <div class="m-portlet m-portlet--mobile">
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::ItemComponent.components.ItemComponentFilterForm')
                    ...
                    <section class="content table-responsive">
                      <table id="data-table" class="table table-striped- table-bordered table-hover table-checkable" >
                          ...
                          <tr>
                              ...
                              <th> سعر الوحده </th>
                              <th>القيمه</th>
                              <th>الكميه المنتجه</th>
                              <th>التكلفه المباشره</th>
                              <th>5% أهلاكات</th>
                              <th> نشأ في</th>
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.ItemComponent.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.ItemComponent.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
