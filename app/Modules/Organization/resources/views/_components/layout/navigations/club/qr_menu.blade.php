...
<div class="row">
    <div class="col-xl-12">
        ...
        <div class="d-flex flex-wrap flex-row bd-highlight mb-3">
            <!-- Start QR Menu  Module -->

            @if (checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),
                'QrMenu-Add'))
                <a href="{{ route('organizations.qrMenu.create') }}" class="menu_box">
                    <center>
            ...
            @if (checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),
                'QrMenu-View'))
                <a href="{{ route('organizations.qrMenu.index') . '?view=view' }}" class="menu_box">
                    <center>
            ...
            @if (checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),
                'QrMenu-Delete'))
                <a href="{{ route('organizations.qrMenu.index') . '?view=trash' }}" class="menu_box">
                    <center>
