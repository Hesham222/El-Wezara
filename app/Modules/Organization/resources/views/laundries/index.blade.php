...
@else
  <x-slot name="pageTitle">المغاسل | عرض</x-slot name="pageTitle">
        ...
        <div class="m-portlet m-portlet--mobile">
          <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                  @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'Laundry-Add'))

                  <li class="m-portlet__nav-item">
                  <a href="{{route('organizations.laundry.create')}}"
                    class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                    <span>
                      <i class="la la-plus">
                      </i>
                      <span>أضف مغسلة جديد</span>
                    </span>
                  </a>
                </li>
                  @endif
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::laundries.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.laundry.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.laundry.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
