...
@if(count($records))
    @foreach($records as $record)
        <tr id="tableRecord-{{$record->id}}">
            ...
            <td>{{$record->Subscription?$record->Subscription->pricing_name:"لا يوجد"}}</td>
            ...
            <td>
                @if($record->approved == 0)
                  <a href="{{route('organizations.report.sportsActivitiesApprove', $record->id)}}" class="btn btn-primary">قبول</a>               
                @endif
            </td>
        </tr>
    @endforeach
