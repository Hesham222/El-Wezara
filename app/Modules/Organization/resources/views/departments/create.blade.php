<x-organization::layout>
    <x-slot name="pageTitle">@lang('Organization::organization.department') | @lang('Organization::organization.create')</x-slot name="pageTitle">
    @section('department-active', 'm-menu__item--active m-menu__item--open')
    @section('department-create-active', 'm-menu__item--active')
    <x-slot name="style">
        <!-- Some styles -->
        <style>
            .invalid-feedback {
            ...
            <div class="m-portlet__body">
                <div class="table-responsive">
                    <section class="content">
                        <form method="POST" action="{{route('organizations.department.store')}}"
                                ...
                                <div class="form-group m-form__group row">
                                    <div class="col-lg-6">
                                        ...
                                        <select name="parent_id"  class="form-control m-input m-input--square" id="exampleSelect1">
                                            <option value="">-- اخرت ادارة  --</option>
                                            @foreach($depts as $dept)
                                                <option @if(old('parent_id') == $dept->id) selected @endif value="{{ $dept->id }}">{{ $dept->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('parent_id')
                                        <span class="invalid-feedback" role="alert">
                                ...
                                <div class="form-group m-form__group row">
                                    <div class="col-lg-6">
                                        ...
                                        <select name="employee" required="" class="form-control m-input m-input--square" id="exampleSelect1">
                                           <option value="0">--  اختر رئيس الادارة --</option>
                                            @foreach($emps as $emp)
                                                <option @if(old('employee') == $emp->id) selected @endif value="{{ $emp->id }}">{{ $emp->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('employee')
                                        <span class="invalid-feedback" role="alert">
