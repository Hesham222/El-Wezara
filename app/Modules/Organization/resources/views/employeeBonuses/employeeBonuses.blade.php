<x-organization::layout>
    ...
    @else
        ...
        @section('employeeBonus-indexDepartment-active', 'm-menu__item--active')
    @endif
...
{{--                                    <th>@lang('Organization::organization.actions')</th>--}}
    ...
    <x-slot name="scripts">
        <script>
            $(function () {
                render("{!! route('organizations.employeeBonus.empBonusData',['id'=>$emp->id,'view' => request()->input('view',0)]) !!}");
                /**
                ...
                $('#searchButton').on('click', function (event) {
                    event.stopPropagation();
                    render("{!! route('organizations.employeeBonus.empBonusData',['id'=>$emp->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
                });
            });
        </script>
        <!-- external JS -->
