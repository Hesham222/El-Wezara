<x-organization::layout>
  ...
  <x-slot name="pageTitle">سجل قيود - المرحله  | سله المهملات</x-slot name="pageTitle">
  @section('dailyAccounts-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">سجل قيود - المرحله | عرض</x-slot name="pageTitle">
  @section('dailyAccounts-view-active', 'm-menu__item--active')
@endif
        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                <li class="m-portlet__nav-item">
                  <a href="{{route('organizations.dailyAccount.create')}}"
                    class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                    <span>
                      <i class="la la-eye">
                      </i>
                      <span> سجل قيود اليوم</span>
                    </span>
                  </a>
                </li>
              </ul>
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::dailyAccounts.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.dailyAccount.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.dailyAccount.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
