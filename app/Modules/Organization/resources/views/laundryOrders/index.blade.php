<x-organization::layout>
  ...
  <x-slot name="pageTitle">طلبات المغاسل  | سله المهملات</x-slot name="pageTitle">
  @section('laundryOrders-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">طلبات المغاسل | عرض</x-slot name="pageTitle">
  @section('laundryOrders-view-active', 'm-menu__item--active')
@endif
        ...
        <div class="m-portlet m-portlet--mobile">
          <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                  @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'LaundryOrder-Add'))

                  <li class="m-portlet__nav-item">
                  <a href="{{route('organizations.laundryOrder.create')}}"
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::laundryOrders.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.laundryOrder.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.laundryOrder.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
