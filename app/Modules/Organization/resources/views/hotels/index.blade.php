<x-organization::layout>
  ...
  <x-slot name="pageTitle">الفنادق  | سله المهملات</x-slot name="pageTitle">
  @section('hotels-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">الفنادق | عرض</x-slot name="pageTitle">
          ...
          <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                  @if(checkOrganizationAdminPermission(auth('organization_admin')->user()->role->permissions->pluck('permission_id')->toArray(),'Hotel-Add'))

                  <li class="m-portlet__nav-item">
                  <a href="{{route('organizations.hotel.create')}}"
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.hotel.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.hotel.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
