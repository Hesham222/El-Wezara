<x-organization::layout>
  ...
  <x-slot name="pageTitle">الفندق - جرد  | سله المهملات</x-slot name="pageTitle">
  @section('hotels-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">الفندق | جرد</x-slot name="pageTitle">
          ...
          <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
             جرودات الفندق
                [{{$area->name}}]
                <br>
                <a href="{{route('organizations.HotelStocking.create',$area->id)}}" class="btn btn-primary">عمل جرد جديد</a>
            </div>
          </div>
          <div class="m-portlet__body">
             <section class="content">
                @include('Organization::HotelStockings.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.HotelStocking.data',['id'=>$area->id,'view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.HotelStocking.data',['id'=>$area->id,'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
</x-slot>
