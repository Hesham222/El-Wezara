<x-organization::layout>
  ...
  <x-slot name="pageTitle">عدد التدريبات | عرض</x-slot name="pageTitle">
  @section('trainerAttends-view-active', 'm-menu__item--active')

@section('trainerAttends-active', 'm-menu__item--active m-menu__item--open')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('organizations.trainerAttend.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('organizations.trainerAttend.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val()+'&day=' + $('#dayCol').val());
        });
    });
  </script>
</x-slot>
