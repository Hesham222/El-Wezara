...
class FilterAction
{
    public function execute(Request $request)
    {
        ...
        if ($request->has('searchKey')){
            ...
            return Ingredient::
            where('main',1)->
            select(['id','name','re_order_quantity','selling_price','ingredient_category_id','last_selling_price','description','quantity','stock','re_order_quantity','final_cost','cost','price','unit_measurement_id','deleted_by','created_at'])
         ...
         when($request->input('view') == 'trash', function ($query) use ($request) {
            ...
            ->select(['id','name', 'description','re_order_quantity','ingredient_category_id','selling_price','last_selling_price','quantity','stock','final_cost','cost','price','unit_measurement_id','deleted_by','created_at'])
