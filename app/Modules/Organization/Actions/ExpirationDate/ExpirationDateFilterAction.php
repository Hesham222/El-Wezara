<?php
namespace Organization\Actions\ExpirationDate;
use Illuminate\Http\Request;
use Carbon\Carbon;
use Organization\Models\Ingredient;

class ExpirationDateFilterAction
{
    public function execute(Request $request)
    {
        ...
        ->when($request->input('column') && $request->input('value'), function ($query) use ($request){
            ...
            ->when($request->input('column') == 'ingredient', function ($query) use ($request){
                    return $query->where('name->ar', 'like', '%' . $request->input('value') . '%');
            });
        });
    }
}
