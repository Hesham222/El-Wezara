...
class FilterAction
{
    ...
    {
        return ExternalPayment::when($request->input('subscriber'), function ($query) use ($request){
        ...
        ->with('ExternalReservation')
        ->select(['id','subscriber_id','external_reservation_id','payment_amount','payment_method','deleted_by','deleted_at', 'created_at'])
