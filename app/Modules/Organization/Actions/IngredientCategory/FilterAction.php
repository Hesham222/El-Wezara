...
class FilterAction
{
    ...
    {
        return IngredientCategory::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])->with('Parent')
        ->select(['id','name','parent_id','description','deleted_by','deleted_at', 'created_at'])
