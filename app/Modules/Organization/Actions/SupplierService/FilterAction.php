...
class FilterAction
{
    ...
    {
        return SupplierService::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->select(['id','name','price','club_commission','description','vendor_id','deleted_by','deleted_at', 'created_at'])
        ...
        ->when($request->input('column') && $request->input('value'), function ($query) use ($request){
            return $query
                ...
                ->when($request->input('column') == 'supplier_name' , function ($query) use ($request){
