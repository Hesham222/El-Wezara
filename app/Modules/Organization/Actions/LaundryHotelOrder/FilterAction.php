...
class FilterAction
{
    ...
    {
        return LaundryHotelOrder::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->select(['id','customer_name','customer_mobile','laundry_id','total_price','date','time','deleted_by','deleted_at', 'created_at'])
