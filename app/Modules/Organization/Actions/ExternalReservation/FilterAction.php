...
class FilterAction
{
    ...
    {
        return ExternalReservation::when($request->input('subscriber'), function ($query) use ($request){
        ...
        ->with('ExternalPricing')
            ->with(['Subscriber' => function ($query) use ($request) {
                $query->select(['id','name']);
            }])
        ->select(['id','subscriber_id','external_pricing_id','num_of_hours','date','start_time','end_time','total','deleted_by','deleted_at', 'created_at'])
