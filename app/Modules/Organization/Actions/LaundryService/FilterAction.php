...
class FilterAction
{
    ...
    {
        return LService::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
//        ->with(['laundry' => function ($query) use ($request) {
//                $query->select(['id','name']);
//        }])
        ->select(['id','name','description','deleted_by','deleted_at', 'created_at'])
