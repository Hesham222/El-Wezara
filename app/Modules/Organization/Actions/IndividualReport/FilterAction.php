<?php
namespace Organization\Actions\IndividualReport;
...
class FilterAction
{
    ...
    {
        return Hotel::whereHas('hotelReservations',function ($query) use ($request) {
            $query->where('supplier_id',null)->whereBetween('created_at', [$request->input('date_from'), $request->input('date_to')]);
