...
class FilterAction
{
    public function execute(Request $request)
    {
        ...
        ->with(['SubscriberType' => function ($query) use ($request) {
                $query->select(['id','type']);
        }])
        ->select(['id','name','subscriber_type_id','phone','second_phone','attachment','national_id','deleted_by','deleted_at', 'created_at'])
