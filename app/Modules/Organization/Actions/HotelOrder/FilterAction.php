...
class FilterAction
{
    ...
    {
        return HotelOrder::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->select(['id','hotel_id','status','created_by','rejection_reason','deleted_by','deleted_at', 'created_at'])
