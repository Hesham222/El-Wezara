...
use Organization\Models\VendorType;

class FilterAction
{
    public function execute(Request $request)
    {
        return VendorType::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->select(['id','name','deleted_by','deleted_at', 'created_at'])
