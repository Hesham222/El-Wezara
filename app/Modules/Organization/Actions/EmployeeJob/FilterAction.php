...
class FilterAction
{
    ...
    {
        return EmployeeJob::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])

            ->select(['id','name', 'description','Vacation_balance','deleted_by','deleted_at','created_at'])
