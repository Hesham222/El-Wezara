...
class FilterAction
{
    ...
    {
        return GateShift::with('gateShiftAdmins')->when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->select(['id','gate_id','week_day_id','created_by','deleted_by','deleted_at', 'created_at'])
