...
class FilterAction
{
    ...
    {
        return JournalEntry::when($request->input('view') == 'trash', function ($query) use ($request) {
        ...
        ->select(['id','description','status','sum_debits','sum_credits','created_by','deleted_by','deleted_at', 'created_at'])
        ...
        ->when($request->input('column') && $request->input('value'), function ($query) use ($request){
            return $query->when($request->input('column') == 'deletedBy' , function ($query) use ($request){
                ...
                ->when($request->input('column') == 'name', function ($query) use ($request){
                    return $query->where('description', 'like', '%' . $request->input('value') . '%');
                });
        });
