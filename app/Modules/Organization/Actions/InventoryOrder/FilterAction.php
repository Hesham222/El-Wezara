...
class FilterAction
{
    ...
    {
        return InventoryOrder::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->select(['id','laundry_id','status','rejection_reason','created_by','deleted_by','deleted_at', 'created_at'])
