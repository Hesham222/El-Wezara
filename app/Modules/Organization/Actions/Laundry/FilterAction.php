...
class FilterAction
{
    ...
    {
        return laundry::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->with('employees')
        ->with(['head' => function ($query) use ($request) {
                $query->select(['id','name']);
        }])
        ->select(['id','name','head_id','description','deleted_by','deleted_at', 'created_at'])
