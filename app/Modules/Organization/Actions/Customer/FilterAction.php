...
class FilterAction
{
    ...
    {
        return Customer::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->with(['CustomerType' => function ($query) use ($request) {
                $query->select(['id','name']);
        }])
        ->select(['id','name','customerType_id','trouble_maker','phone','email','gender','address','date_of_birth','nationality','deleted_by','deleted_at', 'created_at'])
