...
class FilterAction
{
    ...
    {
        return SubAssetProduct::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->select(['id','name','assetProduct_id','start_value','current_value','entry_date','deleted_by','deleted_at', 'created_at'])
