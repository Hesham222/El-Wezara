...
class FilterAction
{
    ...
    {
        return CustomerComplain::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])
        ->select(['id','customer_id','complain','created_by','deleted_by','deleted_at', 'created_at'])
