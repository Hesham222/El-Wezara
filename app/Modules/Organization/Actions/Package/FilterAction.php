...
class FilterAction
{
    ...
    {
        return Package::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])->with(['hall' => function ($query) use ($request) {
             $query->select(['id','name']);
        }])
        ->select(['id','name','hall_id','actual_price','final_price','capacity','description','deleted_by','deleted_at', 'created_at'])
        ...
        ->when($request->input('column') && $request->input('value'), function ($query) use ($request){
            return $query->when($request->input('column') == 'deletedBy' , function ($query) use ($request){
                ...
                ->when($request->input('column') == 'capacity', function ($query) use ($request){
                    return $query->where('capacity', $request->input('value'));
                })
                ->when($request->input('column') == 'hall' , function ($query) use ($request){
                return $query->whereHas('hall', function ($query) use ($request) {
