...
class FilterAction
{
    ...
    {
        return LaundrySubCategory::when($request->input('view') == 'trash', function ($query) use ($request) {
            return $query->onlyTrashed();
        })->with(['deletedBy' => function ($query) use ($request) {
            $query->select(['id','name']);
        }])

        ->select(['id','parent_id','name','description','deleted_by','deleted_at', 'created_at'])
        ->when($request->input('start_date') && $request->input('end_date'), function ($query) use ($request) {
            return $query->whereBetween('created_at',[Carbon::parse($request->input('start_date')), Carbon::parse($request->input('end_date'))]);
        })
            //sub query used in search field
            ->when($request->input('parentCol') != null, function ($query) use ($request){
                return $query->where('parent_id',$request->input('parentCol'));
            })
            ...
            ->when($request->input('column') && $request->input('value'), function ($query) use ($request){
                return $query
                 ...
                 ->when($request->input('column') == 'name', function ($query) use ($request){
                    ...
                    ->when($request->input('column') == 'parent',function ($query) use ($request){
                     return $query->whereHas('parent', function ($query) use ($request) {
                            $query->where('name', 'like', '%' . $request->input('value') . '%');
                     });
                 });
        });
